...
public class RungeKutta2Calculator extends JFrame {
    private JTextField functionInput, initialXInput, initialYInput, stepSizeInput, iterationsInput;
    private JTextArea resultArea;

    public RungeKutta2Calculator(JFrame parent) {
        super("Método Runge-Kutta de Segundo Orden (RK2)");
        setSize(500, 400);
        ...
        // Botón para calcular
        JButton calculateButton = new JButton("Calcular RK2");
        configureButton(calculateButton, this::calculateRK2);
    ...
    private void calculateRK2(ActionEvent e) {
        try {
            ...
            steps.append("Método Runge-Kutta de Segundo Orden (RK2):\n");
            for (int i = 0; i < n; i++) {
                double k1 = h * evaluate(function, x, y);
                double k2 = h * evaluate(function, x + h, y + k1);
                y += (k1 + k2) / 2;
                x += h;
