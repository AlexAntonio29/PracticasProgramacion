...
public class AdamsMoultonCalculator extends JFrame {
    private JTextField functionInput, initialXInput, initialYInput, stepSizeInput, iterationsInput;
    private JTextArea resultArea;

    public AdamsMoultonCalculator(JFrame parent) {
        super("Método Adams-Moulton");
        setSize(500, 400);
        ...
        // Botón para calcular
        JButton calculateButton = new JButton("Calcular Adams-Moulton");
        configureButton(calculateButton, this::calculateAdamsMoulton);
    ...
    private void calculateAdamsMoulton(ActionEvent e) {
        try {
            ...
            steps.append("Método Adams-Moulton:\n");

            double[] fValues = new double[n + 1];

            for (int i = 0; i < n; i++) {
                fValues[i] = evaluate(function, x, y);
                double predictor = y + h * (1.5 * fValues[i] - 0.5 * (i > 0 ? fValues[i - 1] : fValues[i]));
                double corrector = y + h * (0.5 * fValues[i] + 0.5 * evaluate(function, x + h, predictor));
                y = corrector;
                x += h;
