...
public class ImprovedEulerMethodCalculator extends JFrame {
    private JTextField functionInput, initialXInput, initialYInput, stepSizeInput, iterationsInput;
    private JTextArea resultArea;

    public ImprovedEulerMethodCalculator(JFrame parent) {
        super("Método de Euler Mejorado (Heun) - Solución EDO");
        setSize(500, 400);
        ...
        // Botón para calcular
        JButton calculateButton = new JButton("Calcular Euler Mejorado");
        configureButton(calculateButton, this::calculateImprovedEuler);
    ...
    private void calculateImprovedEuler(ActionEvent e) {
        try {
            ...
            steps.append("Método de Euler Mejorado (Heun):\n");
            for (int i = 0; i < n; i++) {
                double fxy = evaluate(function, x, y);
                double predictor = y + h * fxy;
                double corrector = y + (h / 2) * (fxy + evaluate(function, x + h, predictor));
                y = corrector;
                x = x + h;
