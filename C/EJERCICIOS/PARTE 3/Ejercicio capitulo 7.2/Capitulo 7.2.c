//trabajador3Archivo.c   Ejercicio 7.2		Alexis Zosimo Antonio Angel    102-B
#include<stdio.h>
...
void guardarTrabajador(Trabajador t[], int n){
	...
	if(p!=NULL){
		...
		i=0;
		while(t[i].clave != 0){
			fwrite(&t[i],sizeof(Trabajador),1,p);
			i++;
		}
		fclose(p);
	} else {
...
void abrirTrabajador (Trabajador t[], int n){
	...
	if(p!=NULL){
		...
		while(!feof(p)){
			fread(&t[i],sizeof(Trabajador),1,p);
			i++;
		}
	}else{
